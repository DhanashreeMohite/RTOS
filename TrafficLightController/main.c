...
osStatus_t mutexStatusA,mutexStatusB;
...
osTimerId_t irqAId,irqBId;

void irqA (void *argument); 
void irqB (void *argument); 

osThreadId_t tid_irqA;
osThreadId_t tid_app_main;
osThreadId_t tid_irqB;
...
int Init_Thread (void) 
{
  ...
  if (!tid_irqA) return(5);
	...
	tid_irqB = osThreadNew (irqB, NULL, NULL);
  if (!tid_irqB) return(6);
	
	status = osThreadSetPriority(tid_irqA,osPriorityHigh2);
	status = osThreadSetPriority(tid_irqB,osPriorityHigh1);
	
  return(0);
}


void idleCallbackA(void *arg) 
{
	return;
}

void idleCallbackB(void *arg) 
{
	return;
}
...
void Init_Timer(void)
{
	...
	irqAId = osTimerNew (idleCallbackA, osTimerPeriodic, NULL,NULL);
	irqBId = osTimerNew (idleCallbackB, osTimerPeriodic, NULL,NULL);
}
  ...
  while (1) 
	{
		...
		{	
			status = osThreadSuspend(tid_app_main);
			mutexStatusA = osMutexAcquire(mutex_id, osWaitForever);
			osDelay(1000);
			if(mutexStatusA == osOK)
			{
				PTC->PCOR |=	((1U <<  5)|(1U <<  9)|(1U <<  7)|(1U <<  0));
				PTB->PSOR |= ((1U <<  18)|(1U <<  2)) ;  
				PTB->PCOR |= ((1U <<  19)|(1U <<  3)| (1U <<  10));
				PTC->PCOR = (1U <<  1) ;
				osTimerStart (irqAId, delay);
				osDelay(delay);
				mutexStatusA = osMutexRelease(mutex_id);
				osDelay(500);
				status = osThreadResume(tid_app_main);
			}
...
void irqB (void *argument) 
{
  while (1) 
	{
		if((PTD->PDIR | 0XFFF7)== 0xFFFF)
		{	
			status = osThreadSuspend(tid_app_main);
			mutexStatusB = osMutexAcquire(mutex_id, osWaitForever);
			osDelay(1000);
			if(mutexStatusB == osOK)
			{
				PTC->PCOR |=	((1U <<  5)|(1U <<  9)|(1U <<  7)|(1U <<  0));
				PTB->PSOR |= ((1U <<  18) | (1U <<  2)|(1U <<  10));  
				PTB->PCOR |= ((1U <<  3) |(1U <<  19));
				PTC->PSOR |=(1U <<  1);
				osTimerStart (irqBId, delay);
				osDelay(delay);
				mutexStatusB = osMutexRelease(mutex_id);
				osDelay(500);
				status = osThreadResume(tid_app_main);
			}
		} 
